...
const Hero: React.FC = () => {
  ...
  const [isBlinkAnimationActive, setIsBlinkAnimationActive] = useState(true);

  // Idle winking logic
  useEffect(() => {
    const idleWinkInterval = setInterval(() => {
      if (!isWinking) {
        // Pause blink BEFORE wink starts
        setIsBlinkAnimationActive(false);
        setIsIdleWinking(true);
        setTimeout(() => {
          setIsIdleWinking(false);
          // Resume blink AFTER wink fade out
          setTimeout(() => setIsBlinkAnimationActive(true), 300); // 300ms matches fade-out
        }, 800); // Wink duration
      }
    }, 6000);
  ...
  const handleTurtleHover = () => {
    // Pause blink before showing wink
    setIsBlinkAnimationActive(false);
    setIsWinking(true);
    setIsIdleWinking(false);
  };
  ...
  const handleTurtleLeave = () => {
    setIsWinking(false);
    // Resume blink after fade transition
    setTimeout(() => setIsBlinkAnimationActive(true), 300);
  };
